...
from tensorflow.keras.layers import Conv2D, BatchNormalization, Activation, Dense, Concatenate, Flatten, Reshape, Dropout, Activation
from tensorflow.keras.regularizers import l2
import larq_zoo as lqz
import larq as lq

source_layers_to_extract = {
    'QuickNet':['activation_2','activation_3','tf_op_layer_AddV2_15'],
...
extra_layers_params = [[(128, 1, 1, 'same'), (256, 3, 2, 'same')],
                       [(128, 1, 1, 'same'), (256, 3, 1, 'valid')],#valid
                       [(128, 1, 1, 'same'), (256, 3, 1, 'valid')]]#valid
...
def create_base_model(base_model_name, pretrained=True, IMAGE_SIZE=[300, 300]):
    ...
    if base_model_name == "QuickNet":

        base = lqz.sota.QuickNet(input_shape=(*IMAGE_SIZE ,3),include_top=False,weights=None)
        temp = Activation("relu",name="activation_4")(base.layers[-1].output)
        base = keras.Model(base.input,temp)
        if(weights!=None):
            base.set_weights(lqz.sota.QuickNet(include_top=False).get_weights())
